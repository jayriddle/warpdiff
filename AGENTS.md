...
- `js/wipe.js` — Stack-mode real-layer wipe controller (`_wipe*` state, pair cycling, explicit Left/Right analysis target, side-aware loupe hit geometry, divider pointer/keyboard handling, shared reveal geometry). Scopes and rotation use the explicit target; the loupe follows the visible side under the cursor. **Stateful** (shares globals). Currently gated to images; the DOM/clip-path architecture is intended for later video compositor validation.
