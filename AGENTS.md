...
- `playAllMedia` routes through `_applyNativeLoopPolicy`; native loop is enabled only for current transport participants when no managed loop region is active. Otherwise native loop fights the RVFC seek-back.
