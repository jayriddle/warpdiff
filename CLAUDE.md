...
- `js/scrub-video.js` — WebCodecs scrub decoder sessions (`_createScrubVideoSession`): per-slot VideoDecoder, GOP-run feeding with reorder margin, rAF-coalesced painting, decoded-frame ImageBitmap cache (distance-gated: frames the eviction policy would immediately discard are never cloned/bitmapped — reverse scrubs re-decode whole GOPs and would otherwise churn dozens of `createImageBitmap`s per step). Sessions **suspend** on playback start (`suspend()` closes only the VideoDecoder — the hardware pipeline that starves playing `<video>`s — keeping file bytes, demux, and frame cache; `request()` lazily resumes in ~ms) and fully `close()` only on media clear. Consumed by the overlay controller in `index.html` (`_suspendScrubSessions` from `playAllMedia` vs `_releaseScrubSessions` from `clearAllMedia`). No app-state deps (canvas injected via `attach`).
...
- `tests/` — `warpdiff.spec.ts` (Playwright) + `ownership.test.mjs` (dependency-free Node harness; see Testing). `tests/global-setup.ts` auto-generates the gitignored MP4/WebM/WAV/MP3 fixtures via `tests/fixtures/generate.sh` on first run (requires `ffmpeg` on PATH). The H.264 MP4 fixtures need a proprietary-codec Chrome; the VP9+Vorbis WebM pair exists so the 2026-07 regression suites (scrub-drag recovery, pause snap, switch repaint) also run on open-codec Chromium builds — Vorbis deliberately, since Opus slots take the Web Audio path where `.muted` is always true and the muted-flag tests would be vacuous.
...
### Other
- **`endScrubDrag(e)`** (in the scrub closure in `index.html`) is the sole drag finalizer — mouseup routes through it, and so do three **lost-mouseup recovery** paths: mousemove with `e.buttons === 0` (release happened outside the window), `window` blur (Alt-Tab mid-drag), and a mousedown arriving while `isDragging` is still true. A stuck `isDragging` is catastrophic: `playAllMedia` stops suspending scrub decoders (choppy playback), the drift lock stands down permanently, and the muted-state snapshot is never restored — the next mousedown then saves all-unmuted as the snapshot, so every later scrub restores every video unmuted (double audio). The snapshot save is additionally guarded (`if (!_scrubMutedStates)`) so an escaped stuck drag can't clobber it. `e === null` means "no pointer position" (recovery): position-dependent final seeks are skipped and videos settle on `_scrubOverlayLastT`. Guarded by ownership harness (L).
...
- `playwright.config.ts` declares `globalSetup: './tests/global-setup.ts'`, which auto-generates the gitignored MP4 / WebM / WAV / MP3 fixtures via `tests/fixtures/generate.sh` if `tests/fixtures/landscape_a.mp4` or `tests/fixtures/vorbis_a.webm` is missing. Requires `ffmpeg` on PATH; setup errors out with an install hint if it isn't.
