...
- **`_driftLockTick(primary)`** (called every rAF from `startProgressUpdateLoop`) holds followers on the primary's clock: Stack-mode followers are `display:none`, so they hard-seek whenever off by more than half a frame of the coarser of the two grids (invisible); Grid-mode followers are on screen, so they converge via ±2% `playbackRate` nudges (inaudible — every non-active slot is muted) with a 150 ms hard-seek backstop. The engage threshold is computed **per follower** at `min(primary fps, follower fps)` so a low-fps follower under a high-fps primary isn't re-seeked every tick (thrash). The nudged rate is re-asserted each tick so a J/K rate write mid-episode can't strand a follower at the base rate; it stands down during `isDragging` / `_bulkSyncActive`. The clock-master base is read from the user-selected rate (`PLAYBACK_RATES[playbackRateIndex]`), not `primary.playbackRate`, and the primary is forced onto it (nudge cleared) each tick — otherwise an asset switch that promotes a mid-nudge follower to primary would strand the whole cluster at ±2% (a global tempo skew, audit finding C1).
